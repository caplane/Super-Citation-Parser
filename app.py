...
def fetch_web_metadata(url):
    ...
    try:
        ...
        if path.endswith('.pdf'):
            ...
            return [{'type': result_type, 'title': title, 'authors': [], 'publisher': 'U.S. Government' if is_gov else '', 'year': '', 'url': url, 'short_link': short_link, 'domain': domain, 'access_date': datetime.now().strftime("%B %d, %Y"), 'id': 'web_pdf_result'}]
        ...
        return [{'type': 'gov' if domain.endswith('.gov') else 'web', 'title': "Web Resource", 'authors': [], 'publisher': '', 'year': '', 'url': url, 'short_link': None, 'domain': domain, 'access_date': datetime.now().strftime("%B %d, %Y"), 'id': 'web_result_failed'}]
